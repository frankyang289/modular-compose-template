...
val DefaultRoborazziOptions =
    RoborazziOptions(
        compareOptions = RoborazziOptions.CompareOptions(changeThreshold = 0.85f),
...
fun <A : ComponentActivity> AndroidComposeTestRule<ActivityScenarioRule<A>, A>.captureForDevice(
    ...
    // Run accessibility checks first so logging is included in the screenshot output
    ...
    // Rethrow the accessibility exception once the screenshot has been captured
 ...
 * is enabled.
 *
 * Requires API 31+ (Android S) because [AppTheme] uses dynamic color APIs. Annotate your test
 * class with `@Config(sdk = [Build.VERSION_CODES.S])` or higher when using this function.
 */
@RequiresApi(31)
    ...
    darkModeValues.forEach { isDarkMode ->
        ...
        androidThemeValues.forEach { isAndroidTheme ->
            ...
            dynamicThemingValues.forEach dynamicTheme@{ isDynamicTheming ->
                // Skip combinations of Android Theme + Dynamic color as they are incompatible
